		...
		if isinstance(self, ParameterRef):
			...
			if isinstance(other, LinearComponent):
				...
				if other.param == _null_:
					return LinearComponent(param=str(self), data=str(other.data), scale=other.scale)
				from .linear_math import ParameterMultiply
				return ParameterMultiply(self, other)
		elif isinstance(other, ParameterRef):
...
class DataRef(UnicodeRef_):
	...
	def __add__(self, other):
		if isinstance(self, (DataRef, _Number)) and isinstance(other, (DataRef, _Number)):
		...
		if isinstance(self, (DataRef, _Number)) and isinstance(other, str) and not isinstance(other, DataRef):
			raise TypeError(_unsupported_operands('-', self, other))
		if isinstance(other, (DataRef, _Number)) and isinstance(self, str) and not isinstance(self, DataRef):
			raise TypeError(_unsupported_operands('-', self, other))
	...
	def __mul__(self, other):
		if isinstance(self, DataRef):
			...
			if isinstance(other, ParameterRef):
				return LinearComponent(param=str(other), data=str(self))
			if isinstance(other, LinearComponent):
				return LinearComponent(param=str(other.param), data=str(self * other.data), scale=other.scale )
			...
			if isinstance(self, LinearComponent):
				return LinearComponent(param=str(self.param), data=str(self.data * other) )
	...
	def __sub__(self, other):
		if isinstance(self, LinearComponent):
			if other == () or other == 0:
				...
				return LinearFunction([self, -LinearComponent(param=_null_, data=str(other))])
			else:
				try:
					return self.as_pmath() - other
				except NotImplementedError:
...
class LinearFunction:
		...
		if init is not None and init != 0:
			for i in init_:
				if isinstance(i, LinearComponent):
					...
					raise TypeError(f'members of {self.__class__.__name__} must be LinearComponent')
	...
	def __mul__(self, other):
		...
		if isinstance(other, LinearFunction) and isinstance(self, _Number):
			...
			return trial
		if isinstance(other, LinearFunction) and isinstance(self, (ParameterRef, _ParameterOp)):
			if isinstance(self, ParameterRef):
				if self == _null_:
					return other
				trial = LinearFunction()
	...
	def __truediv__(self, other):
		...
		if isinstance(self, LinearFunction) and isinstance(other, (ParameterRef, _ParameterOp, _Number)):
			return ParameterDivide(self.as_pmath(), other)
		if isinstance(other, LinearFunction) and isinstance(self, (ParameterRef, _ParameterOp, _Number)):
	...
	def reformat_param(self, container=None, pattern=None, repl=None, **kwargs):
		...
		for i in self:
			...
			if container is None:
				container = '{}'
			r += LinearComponent(data=str(i.data), param=container.format(param), scale=i.scale)
		return r
	...
	def reformat_data(self, container=None, pattern=None, repl=None, **kwargs):
		...
		for i in self:
			...
			if container is None:
				container = '{}'
			r += LinearComponent(data=container.format(data), param=str(i.param), scale=i.scale)
		return r
...
class DictOfLinearFunction:
	...
	def __setitem__(self, k, v):
		...
		if isinstance(v, int) and v==0:
			...
			raise TypeError(f"values in {self.__class__} can only have type LinearFunction, not {type(v)}")
