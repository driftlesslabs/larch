...
from .model.linear import LinearComponent, LinearFunction
    ...
    def __mul__(self, other):
        ...
        if type(other) is _param_multiply:
            if isinstance(other._left, ParameterRef):
                return (
                    LinearComponent(data=str(self), param=str(other._left))
                    * other._right
                )
            else:
                return (
                    LinearComponent(data=str(self), param=str(other._right))
                    * other._left
                )
...
class ParameterRef(str, metaclass=Role):
    """
    An abstract reference to a parameter, which may or may not be in any given model.
    ...
    def __new__(
        cls, name, default=None, fmt=None, default_value=None, format=None, scale=None
    ):
    ...
    def fmt(self, format):
        ...
        ----------
        format : str
            The new format string. This will be used to format the parameter
            value when calling :meth:`str`.
        ...
        if type(other) is DataRef:
            if isinstance(self, _param_multiply):
                if isinstance(self._left, ParameterRef):
                    return (
                        LinearComponent(data=str(other), param=str(self._left))
                        * self._right
                    )
                else:
                    return (
                        LinearComponent(data=str(other), param=str(self._right))
                        * self._left
                    )
    ...
    def __le__(self, other):
        from .model.jaxmodel.guidance import ParameterScaledOrdering
    ...
    def __ge__(self, other):
        from .jaxmodel.guidance import ParameterScaledOrdering
    ...
    def lessthan(self, other):
        ...
        from .jaxmodel.guidance import ParameterBound, ParameterScaledOrdering
    ...
    def equalto(self, other):
        ...
        if s is None:
            raise TypeError("self is not scaled parameter")
        from .jaxmodel.guidance import ParameterScaledEquality
...
class _param_math_binaryop(ParameterRef):
    def __new__(cls, left, right):
        ...
        self._left = left
        self._right = right
        if isinstance(self._left, ParameterRef) and isinstance(
            self._right, ParameterRef
        ):
...
class LinearComponent2(tuple):
    ...
    def __mul__(self, other):
        if isinstance(
            ...
            raise TypeError(
                f"unsupported operand type(s) for {self.__class__.__name__}: {type(other)}"
            )
    ...
    def __rmul__(self, other):
        if isinstance(
            ...
            raise TypeError(
                ...
                f"{self.__class__.__name__}: {type(other)}"
            )
    ...
    def __xml__(self, exponentiate_parameter=False, resolve_parameters=None):
        ...
        if resolve_parameters is not None:
            if exponentiate_parameter:
                plabel = resolve_parameters.pvalue(
    ...
    def _evaluate(self, p_getter, x_namespace=None, **kwargs):
        return (
            self.scale
            * p_getter(str(self.param))
            * self.data.eval(namespace=x_namespace, **kwargs)
        )
...
class LinearFunction2(MutableSequence):
        ...
        if init is not None:
            for i in init:
                if isinstance(i, LinearComponent2):
                    ...
                    raise TypeError(
                        f"members of {self.__class__.__name__} must be LinearComponent2"
                    )
    ...
    def reformat_param(self, container=None, pattern=None, repl=None, **kwargs):
        ...
        for i in self:
            ...
            if container is None:
                container = "{}"
            r += LinearComponent2(
                data=i.data, param=container.format(param), scale=i.scale
            )
        try:
    ...
    def reformat_data(self, container=None, pattern=None, repl=None, **kwargs):
        ...
        for i in self:
            ...
            if container is None:
                container = "{}"
            r += LinearComponent2(
                data=container.format(data), param=i.param, scale=i.scale
            )
        try:
            r._x_ident = self._x_ident
        except AttributeError:
    ...
    def _evaluate(self, p_getter, x_namespace=None, **more_x_namespace):
        ...
        return sum(
            j._evaluate(p_getter, x_namespace=x_namespace, **more_x_namespace)
            for j in self
        )
...
class LinearComponent0(TouchNotify):
    def __init__(self, *args, param=None, data=None, scale=None, **kwargs):
        if len(args):
            if isinstance(args[0], LinearComponent0):
                ...
                raise TypeError(
                    "init LinearComponent0 with LinearComponent0 or keyword arguments"
                )
        super().__init__(**kwargs)
    ...
    def __mul__(self, other):
        if isinstance(
            ...
            return LinearComponent0(
                data=self.data, param=self.param, scale=self.scale * other
            )
        elif isinstance(other, (DataRef,)):
            return LinearComponent0(
                data=self.data * other, param=self.param, scale=self.scale
            )
        else:
            raise TypeError(
    ...
    def __deepcopy__(self, memodict):
        return LinearComponent0(
            param=copy.deepcopy(
                self._param if isinstance(self._param, str) else str(self._param),
                memodict,
            ),
            data=copy.deepcopy(
                self._data if isinstance(self._data, str) else str(self._data), memodict
            ),
...
def multiple_linear_plot_2d(
    ...
    *,
    ...
    svg=True,
    header=None,
    other_namespace=None,
):
    # Delayed evaluation mode...
    if p_getter is None:
        return lambda x: multiple_linear_plot_2d(
            linear_funcs,
            x,
            x_name,
            x_min,
