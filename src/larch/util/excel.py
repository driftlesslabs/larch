...
class ExcelWriter(_XlsxWriter):
    ...
    def add_model(
        ...
        #         if model.dataframes.data_co is not None and len(
        #             model.dataframes.data_co.columns
        #         ):
        #             try:
        ...
        #             model.dataframes.data_ca.columns
        #         ):
        #             try:
        #                 self.add_content_tab(
        ...
        #             model.dataframes.data_ce.columns
        #         ):
        #             try:
        #                 self.add_content_tab(
        ...
        #                     raise
        #         if (
        #             model.dataframes.data_ch is not None
        ...
        if utility_functions:
            try:
                if isinstance(model, ModelGroup):
                    ...
                    self.add_content_tab(
                        model._utility_functions_as_frame(),
                        sheetname="Utility",
        ...
        if nesting and not isinstance(model, ModelGroup) and not model.is_mnl():
            try:
                self.add_content_tab(
                    model.graph.__xml__(output="png"),
                    sheetname="Nesting",
    ...
    def add_metadata(self, key, value):
        ...
        while len(encoded_value):
            self.metadatasheet.write(
                row, c, encoded_value[:30_000]
            )  # excel has cell text length cap
            c += 1
    ...
    def add_content_tab(
        self, content, sheetname, heading=None, startrow=None, blurb=None, to_excel=None
    ):
        if startrow is None:
        ...
        if isinstance(content, (pd.DataFrame, Styler)):
            ...
            if heading is not None:
                ...
                logger.debug(" after heading row is %d", startrow)
            content_.to_excel(
                self, sheet_name=worksheet.name, startrow=startrow, **to_excel
            )
            ...
            if content_data.index.nlevels == 1:
                ...
                new_width = max(
                    current_width, max(len(str(i)) for i in content_data.index)
                )
                ...
                for n in range(content_data.index.nlevels):
                    ...
                    new_width = max(
                        current_width,
                        max(len(str(i)) for i in content_data.index.levels[n]),
                    )
                ...
                try:
                    ...
                    if isinstance(_v, str) and _v[:22] == "data:image/png;base64,":
                        fp, img_size, dpi = make_png(
                            base64.standard_b64decode(_v[22:]),
                            dpi=dpi,
                            output="bytesio",
                            return_size=True,
                            return_dpi=True,
                        ...
                        worksheet.insert_image(
                            startrow, 0, f"image-{sheetname}.png", {"image_data": fp}
                        )
        ...
        if not success and isinstance(content, dict):
            ...
            unpacker(content)
            table_cache = pd.DataFrame(
                data="",
                index=pd.RangeIndex(max_row + 1),
                columns=pd.RangeIndex(max_col + 1),
            )
...
def load_metadata(xlsx_filename, key=None):
    ...
    raw = pandas.read_excel(
        xlsx_filename, sheet_name="_metadata_", index_col=0, engine="openpyxl"
    )
    metadata = {}
    if key is None:
...
def _estimation_statistics_excel(
    ...
    *	Log Likelihood at Constants Only (if known)
    ...
    try:
        ...
        def catname(j):
            ...
            if buffer_cols:
                worksheet.merge_range(
                    row, start_col, row, start_col + buffer_cols, j, bold
                )
            else:
        ...
        if ncases:
            worksheet.merge_range(
                row, datum_col + 1, row, datum_col + 2, ncases, cell_format=comma_0
            )
        else:
            worksheet.merge_range(
                row, datum_col + 1, row, datum_col + 2, "not available"
            )
        row += 1
        ...
        if mostrecent is not None:
            ...
            worksheet.write(
                row, datum_col + 1, mostrecent.loglike, fixed_2
            )  # "{:.2f}".format(mostrecent.loglike)
            if ncases:
                worksheet.write(
                    row, datum_col + 2, mostrecent.loglike / ncases, fixed_4
                )  # "{:.2f}".format(mostrecent.loglike/ ncases)
            else:
        ...
        if ll_z is not None and ll_z != 0:
            ...
            if ncases:
                worksheet.write(
                    row, datum_col + 2, ll_z / ncases, fixed_4
                )  # "{:.2f}".format(ll_z/ ncases)
            else:
                worksheet.write(row, datum_col + 2, "na")
            if mostrecent is not None:
                row += 1
                ...
                worksheet.merge_range(
                    row, datum_col + 1, row, datum_col + 2, rsz, cell_format=fixed_4
                )  # "{:.3f}".format(rsz)
            row += 1
        ...
        if ll_nil is not None and ll_nil != 0:
            ...
            if ncases:
                worksheet.write(
                    row, datum_col + 2, ll_nil / ncases, fixed_4
                )  # "{:.2f}".format(ll_nil/ ncases)
            else:
                worksheet.write(row, datum_col + 2, "na")
            if mostrecent is not None:
                row += 1
        ...
        if ll_c is not None and ll_c != 0:
            ...
            if ncases:
                worksheet.write(
                    row, datum_col + 2, ll_c / ncases, fixed_4
                )  # "{:.2f}".format(ll_c/ ncases)
            else:
                worksheet.write(row, datum_col + 2, "na")
            if mostrecent is not None:
                row += 1
                ...
                worksheet.merge_range(
                    row, datum_col + 1, row, datum_col + 2, rsc, cell_format=fixed_4
                )  # "{:.3f}".format(rsc)
            row += 1
        ...
        if buffer_cols:
            for b in range(buffer_cols):
                proposed_width -= xlsxwriter._col_widths[sheetname].get(
                    start_col + 1 + b, 8
                )
