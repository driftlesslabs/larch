...
def apply_global_background_gradient(
    df, override_min=None, override_max=None, cmap=None, subset=None
):
    if cmap is None:
...
def columnize_with_joinable_backing(
    df, name, inplace=False, dtype=None, debug=False, backing=None
):
    try:
        ...
        if not inplace:
            return columnize(
                df.join(backing), name, inplace=False, dtype=dtype, debug=debug
            )
        else:
            raise
...
def columnize(df, name, inplace=True, dtype=None, debug=False, backing=None):
    ...
    if datanames is not None:
        ...
        for dname in set(datanames):
            out, source, backing = columnize_with_joinable_backing_2(
                source, dname, dtype, backing=backing
            )
...
def counts_and_shares(
    ...
    if isinstance(rows, str):
        if not isinstance(
            tours_df[rows].dtype, pandas.core.dtypes.dtypes.CategoricalDtype
        ):
    ...
    if isinstance(cross, str):
        if bins == "cat":
            pivot_cols = pandas.Series(
                pandas.Categorical(tours_df[cross]), index=tours_df.index
            )
    ...
    if rowtotals:
        ...
        result1.loc["% of TOT", :] = (
            -result1.loc["TOTAL", :] * 2 / result1.loc["TOTAL", :].sum()
        )
    else:
        result1.loc["TOTAL", :] = result1.sum(0)
        result1.loc["% of TOT", :] = (
            -result1.loc["TOTAL", :] / result1.loc["TOTAL", :].sum()
        )
