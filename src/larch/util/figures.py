    ...
    **kwargs,
    ...
    if discrete:
        ...
        discrete, discrete_values = seems_like_discrete_data(
            numpy.asarray(x).reshape(-1), return_uniques=True
        )

    x_discrete_labels = (
        None if discrete_values is None else [str(i) for i in discrete_values]
    )
    ...
    if bins is None:
        ...
        if isinstance(x.dtype, pandas.CategoricalDtype):
            ...
            if isinstance(pct_bins, int):
                bins = numpy.percentile(
                    x_, numpy.linspace(low_pctile, high_pctile, pct_bins + 1)
                )
            else:
    ...
    if style == "kde":
        ...
        kernel_result = scipy.stats.gaussian_kde(
            x, bw_method=bw_method, weights=model_result.reshape(-1)
        )
        common_bw = kernel_result.covariance_factor()
        if model_choice is not None:
            kernel_choice = scipy.stats.gaussian_kde(
                x, bw_method=common_bw, weights=model_choice.reshape(-1)
            )
        else:
            kernel_choice = None
        ...
        if model_choice is not None:
            ...
            if coincidence_ratio:
                _coincidence_ratio = (
                    numpy.minimum(y1, y2).sum() / numpy.maximum(y1, y2).sum()
                )
    ...
    if accumulator and model_choice is not None:
        fig, (ax, ax2) = plt.subplots(
            2, 1, gridspec_kw={"height_ratios": [3, 1], "hspace": 0}, sharex="col"
        )
        ...
        ax2.fill_between(
            x1plus,
            numpy.fmin(cum_sum, 0),
            0,
            facecolor="#ffa200",
            label=f"{prob_label} Ahead",
        )
        ax2.fill_between(
            x1plus,
            numpy.fmax(cum_sum, 0),
            0,
            facecolor="#1f77b4",
            label=f"{obs_label} Ahead",
        )
        ax2.set_yticks([])
...
def distribution_on_idca_variable(
    ...
    **kwargs,
    ...
    if style == "kde":
        ...
        kernel_result = scipy.stats.gaussian_kde(
            x.reshape(-1), bw_method=bw_method, weights=model_result.reshape(-1)
        )
        common_bw = kernel_result.covariance_factor()
        kernel_choice = scipy.stats.gaussian_kde(
            x.reshape(-1), bw_method=common_bw, weights=model_choice.reshape(-1)
        )
    ...
    **kwargs,
    ...
    if style == "dataframe":
        ...
        for n, i in enumerate(h_pr_share.columns):
            x_, y_ = pseudo_bar_data(
                bins - shift, h_pr_share[i], gap=0.2 if discrete else 0
            )
            axes[n].plot(x_, y_, label="Modeled" if n == 0 else None, lw=1.5)

            x_ch_, y_ch_ = pseudo_bar_data(
                bins - shift, h_ch_share[i], gap=0.2 if discrete else 0
            )
            axes[n].fill_between(
                x_ch_,
                y_ch_,
        ...
        legnd = axes[0].legend(
            loc="lower center", ncol=2, borderaxespad=0, bbox_to_anchor=(0.5, 1.08)
        )
...
def distribution_on_idco_variable(
    ...
    **kwargs,
    ...
    if isinstance(x, str):
        ...
        if (
            model.dataframes
            and model.dataframes.data_co is not None
            and x in model.dataframes.data_co
        ):
            x = model.dataframes.data_co[x].values.reshape(-1)
        else:
            x = (
                model.dataservice.make_dataframes({"co": [x]}, explicit=True)
                .array_co()
                .reshape(-1)
            )
    else:
    ...
    for i in range(pr.shape[1]):
        ...
        if (
            i < n_alts
            or include_nests is True
            or model.graph.standard_sort[i] in include_nests
        ):
