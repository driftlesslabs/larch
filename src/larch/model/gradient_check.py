...
def _color_poor_similarity(val):
    """
    Mark poor similarity with red color.
...
def check_gradient(
    ...
    stylize : bool, optional
        If True, return a styled DataFrame with colorized similarity values.
    skip_zeros : bool, optional
        If True, do not include rows where both the finite difference and
        analytic gradients are zero.
    trailing : bool, optional
        If True, the gradient is assumed to be returned in trailing form,
        where the gradient is the last dimension of the array.
